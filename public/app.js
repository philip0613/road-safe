...
function renderRoute(data, sLat, sLon, eLat, eLon, sName, eName) {
  ...
  startMarker = L.marker([sLat, sLon], { icon: createPinIcon('start', '출') }).addTo(map).bindPopup(`<b>출발지:</b> ${sName}`);
  endMarker = L.marker([eLat, eLon], { icon: createPinIcon('end', '도') }).addTo(map).bindPopup(`<b>도착지:</b> ${eName}`);
...
// ==========================================
// 6. Gemini Vision AI 판독 & 민원 등록 (Node 서버 호출)
// ==========================================
