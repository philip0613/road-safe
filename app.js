...
function initMap() {
  ...
  if (typeof L === 'undefined') {
    mapEl.innerHTML = '<div style="color:#ef4444; padding:20px; text-align:center;">지도 엔진을 불러오는 중입니다. 잠시 후 새로고침해 주세요.</div>';
    return;
  }
...
async function searchPedestrianRoute(e) {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`TMAP API 응답 오류 (${res.status})`);
    const data = await res.json();
    renderRoute(data, startLat, startLon, endLat, endLon, startName, endName);
  } catch (err) {
    console.warn('API 응답 불가 시 모의 경로로 표시:', err);
